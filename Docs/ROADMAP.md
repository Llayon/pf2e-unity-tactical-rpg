...
- Expand authored hazards beyond flat entry damage to typed entry effects (`basic save` damage and prone-on-entry variants) while keeping AI pressure data-driven. (Done)
- Add authored hazard support for `PersistentFireOnEntry`, with end-turn ticking fire damage and DC 15 flat-check removal routed through `TurnManager` rather than hazard-local hacks. (Done)
- Add save-gated persistent hazard support (`save -> persistent fire on failure`) on top of the new end-turn ticking seam, so hazards can now gate ongoing fire through a defense check instead of only direct entry application. (Done)
- Add the first mixed multi-stage authored hazard (`basic save` burst damage + `persistent fire on failure`) with separate burst and ongoing-damage payloads. (Done)
- Add the first non-damage multi-stage hazard outcome (`save -> prone + persistent fire on failure`) so authored traps can now combine control and ongoing damage without an immediate burst. (Done)
- Add the first forced-movement authored hazard (`save -> push on failure`) and route it through the existing forced-move path with entry-direction awareness. (Done)
- Add the first forced-move + burst authored hazard (`basic save` damage + `push on failure`) so authored traps can combine immediate punishment with position displacement in one payload. (Done)
- Add the first authored pull trap (`save -> pull on failure`) so hazard displacement is no longer limited to knockback and can now drag targets back toward their entry vector. (Done)
- Add the first forced-move + burst pull hazard (`basic save` damage + `pull on failure`) so authored traps can now punish entry and yank the target back in the same payload. (Done)
- Add the first control + pull authored hazard (`save -> prone + pull on failure`) so traps can now displace and immediately compromise follow-up positioning without direct burst damage. (Done)
- Add the first pull + persistent-condition authored hazard (`save -> pull + persistent fire on failure`) so traps can now combine displacement with an ongoing consequence instead of only immediate control. (Done)
- Add the first multi-condition pull hazard (`save -> pull + prone + persistent fire on failure`) so authored traps can now combine displacement, immediate control, and ongoing pressure in one payload. (Done)
- Add the first multi-condition push hazard (`save -> push + prone + persistent fire on failure`) so the authored hazard matrix is now symmetric across push/pull displacement branches. (Done)
- Formalize multi-cell displacement depth for authored hazards (`forcedMoveCells > 1`) and pin stop rules for occupancy/grid-edge blocking through EditMode coverage. (Done)
- Add the first non-fire persistent authored hazard branch (`save -> persistent acid on failure`) so ongoing trap effects are no longer hardcoded to the fire family. (Done)
- Add the first mixed acid hazard (`basic save` burst + persistent acid on failure`) so the non-fire ongoing branch also covers immediate+ongoing punishment instead of only a pure persistent trigger. (Done)
- Add the first control + acid hazard (`save -> prone + persistent acid on failure`) so the acid branch also covers non-burst control+ongoing outcomes instead of only pure persistent or burst+persistent variants. (Done)

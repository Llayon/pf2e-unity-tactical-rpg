...
- Optional Aid UI fallback wiring in Action Bar is now delegated to `AidActionBarUiBootstrapper` (Aid button discovery/clone, highlight lookup, badge resolve/create/style), reducing view-bootstrap noise inside `ActionBarController` while keeping existing private compatibility entrypoints (`ResolveOptionalAidUiReferences`, `ResolveAidPreparedIndicatorReferences`) for tests and validator contracts.

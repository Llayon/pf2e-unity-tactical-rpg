...
- Presentation now uses a shared roll-text formatter (`RollBreakdownFormatter`) for skill/strike/opposed logs and targeting valid-check hint formulas, keeping source labels and `d20 + mod = total` text style consistent across log and UI hint surfaces.

...
- Aid UI creation policy is now tooling-first: runtime `AidActionBarUiBootstrapper` no longer creates missing `AidButton`/badge objects, and `PF2eSceneDependencyValidator` AutoFix is responsible for creating/wiring missing Aid ActionBar elements in legacy scenes.

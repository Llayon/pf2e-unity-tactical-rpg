...
- Ready Strike arbitration contract: per trigger scope an actor can consume at most one ready reaction (`readyReactionsConsumedInScope`), with explicit scope depth guards to prevent nested trigger double-consume.
...
- Ready trigger orchestration contract (phase 33e): candidate buffering, stale-ready cleanup, initiative-order sorting, and movement/attack trigger dispatch are owned by `ReadyStrikeTriggerOrchestrator`; `TurnManager` should keep only combat-state gating and final trigger execution callback.
- Ready trigger execution contract (phase 33f): prepared-state validation, scoped reaction consumption, and `ReactionBroker.TryExecuteReadiedStrike(...)` invocation are owned by `ReadyStrikeTriggerExecutor`; `TurnManager` should use executor `IsResolving` as nested strike-trigger guard and avoid duplicating execution logic.

...
- Shield flow works at MVP level: `Raise Shield` grants temporary AC via `EntityData` derived stats; `Shield Block` can reduce post-hit damage, damage the shield, and spend reaction. A reusable generic incoming-damage Shield Block path now exists in `ReactionBroker`/`DamageApplicationService` and is wired for `Trip` crit damage.

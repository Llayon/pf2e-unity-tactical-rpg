...
- Targeting UX now includes Action Bar mode highlight plus world-space target feedback (`eligible` highlights + hover valid/invalid tint), a text hint panel explaining preview reasons using the same `TargetingController` validation path as confirm clicks (including a non-blocking warning for ranged `Strike` vs `Concealed` targets), and a two-step targeting state machine for `Reposition` (target -> check -> destination cell on success).
...
1. Extend ranged strike preview warnings beyond `Concealed` (e.g., cover +2 AC risk wording/iconography) and unify “valid with risk” hint formatting.

...
- Ready binder hard-requirement contract (phase 33l): legacy direct-subscribe fallback is removed; if binder is missing, `TurnManager` logs error and does not wire Ready trigger listeners. Validator now treats missing `ReadyStrikeEventBinder` as an error (when `TurnManager` exists).

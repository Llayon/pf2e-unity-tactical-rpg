...
- PlayMode regression now covers multi-round movement/AI/condition-tick flow, blocked-turn recovery, sticky-target lock behavior, ranged concealment/cover logic, Shield Block reaction prompts, and Delay planned-anchor auto-chain, but full matrix coverage for spells/visibility states and complex multi-actor Delay fallback windows is still pending.
...
1. Add focused PlayMode coverage for Delay UI orchestration: planned insertion marker selection, multi-delayed same-anchor chains, and manual fallback Return/Skip window behavior when non-planned delayed actors exist.
2. Continue presentation/domain decoupling by introducing a top-level Delay UI orchestrator that owns ActionBar + InitiativeBar Delay state mediation, reducing remaining Delay-specific wiring inside `InitiativeBarController`.

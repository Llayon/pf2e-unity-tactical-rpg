...
- Ready scene-wiring contract (phase 33k): `ReadyStrikeEventBinder` is scene/tooling-managed (`PF2eSceneDependencyValidator` validate/autofix creates and wires binder on `TurnManager` GO). `TurnManager` no longer creates binder components at runtime; temporary legacy direct bus subscription fallback is retained only when binder is missing.

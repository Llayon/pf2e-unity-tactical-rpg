...
- Weapon-aware strike contract (Phase 25+): `Strike` is the single action path for melee and ranged weapons; validation/runtime branch on equipped weapon (`weapon == null || !IsRanged` => melee, `IsRanged` => ranged). Ranged strikes ignore same-elevation checks in MVP, apply range-increment penalty (`-2` per increment after the first) in addition to MAP, and may apply `Volley` close-range penalty when configured.
...
- Ranged strike MVP is implemented (bow path, range increment penalties, `Volley` penalty, weapon-aware strike targeting, and crit math support for `Deadly`/`Fatal`), but advanced ranged rules remain deferred: ammo, reload, cover, LoS/concealment, volley-info preview UX, striking-rune scaling for crit traits, and crit specialization effects.

...
- Optional Aid UI wiring in Action Bar is delegated to `AidActionBarUiBootstrapper` (Aid button/highlight/badge resolve + style only), reducing view-bootstrap noise inside `ActionBarController` while keeping existing private compatibility entrypoints (`ResolveOptionalAidUiReferences`, `ResolveAidPreparedIndicatorReferences`) for tests and validator contracts.
- Aid UI creation policy is now tooling-only: `PF2eSceneDependencyValidator` AutoFix creates/wires missing Aid ActionBar elements (`AidButton`, `ActiveHighlight`, `AidPreparedBadge`); runtime `AidActionBarUiBootstrapper` no longer creates missing Aid UI hierarchy.
...
- Ready mode-selector wiring contract (phase 33p): scene validator/autofix now owns `ReadyModeSelector` hierarchy creation and wiring (`readyModeSelectorRoot`, `readyModeMoveButton`, `readyModeAttackButton`, `readyModeAnyButton`) under `ReadyButton`; `ActionBarController` emits an explicit warning when selector wiring is missing and fallback runtime bootstrap is used.
- Ready authored-only mode-selector contract (phase 33q): runtime bootstrap for `ReadyModeSelector` is removed from `ActionBarController`; missing selector refs are now treated as validator errors when `readyButton` is present, enforcing scene-authored wiring as the single source of truth.

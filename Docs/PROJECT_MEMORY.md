...
- Encounter authoring now includes per-actor initiative overrides by name (`InitiativeActorOverride` in `EncounterFlowController`/`EncounterFlowUIPreset`): on Start Encounter the list is applied to entities (others are reset to Perception default), enabling scenario-specific stealth openings without code edits.
- Authoring guardrail: unknown initiative override keys emit warnings at Start Encounter (`actorId not found` / `actorName not found`) while preserving normal combat startup.
- Initiative override authoring migrated to stable `actorId` keys: `EntityData.EncounterActorId` is now the primary match key (test actors set to `fighter`, `wizard`, `goblin_1`, `goblin_2`), while legacy `actorName` entries still work temporarily with migration warnings.

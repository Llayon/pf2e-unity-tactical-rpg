...
- PlayMode regression now covers multi-round movement/AI/condition-tick flow, blocked-turn recovery, sticky-target lock behavior, ranged concealment/cover logic, Shield Block reaction prompts, and Delay planned/manual + full-round expiry + pointer-level UI click flows; full matrix coverage for spells/visibility states is still pending.
...
2. Upgrade Delay expiry regression to fully pointer-driven UI setup (Delay + Skip clicks) to harden scene wiring against UI drift.

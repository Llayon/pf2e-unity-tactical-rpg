...
- Aid domain foundation is now available for reaction-based check support: `AidService` (prepare/consume/expire/clear), `AidCheckContext`/`AidOutcome`/`AidPreparedRecord`, typed `AidResolvedEvent` channel on `CombatEventBus`, and `TurnManager` lifecycle integration (`NotifyTurnStarted` expiry + combat-end clear). UI/action wiring for playable Aid remains pending.

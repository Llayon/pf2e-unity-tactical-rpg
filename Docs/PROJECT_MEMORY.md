...
| Typed bus direct-publish tests (EditMode) | Done | EditMode now asserts `TurnManager` publishes combat/turn lifecycle typed events directly to `CombatEventBus` without forwarder adapters |
...
2. Add one more EditMode case for direct typed publish on `EndTurn` path (`TurnEnded` + `ConditionsTicked`) to close the lifecycle loop.

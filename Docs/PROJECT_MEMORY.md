...
- Optional Aid UI fallback wiring in Action Bar is now delegated to `AidActionBarUiBootstrapper` (Aid button/highlight/badge resolve + style), reducing view-bootstrap noise inside `ActionBarController` while keeping existing private compatibility entrypoints (`ResolveOptionalAidUiReferences`, `ResolveAidPreparedIndicatorReferences`) for tests and validator contracts.
- Aid UI creation policy is now tooling-first with compatibility fallback: `PF2eSceneDependencyValidator` AutoFix creates/wires missing Aid ActionBar elements (`AidButton`, `ActiveHighlight`, `AidPreparedBadge`) for legacy scenes, while runtime `AidActionBarUiBootstrapper` still keeps auto-create fallback until scene wiring migration is fully complete.

...
- Player can: start combat, move (Stride), Strike, Trip, Shove, Grapple, Escape, Demoralize, Reposition, Delay, Aid (prepare), Stand, Raise Shield, end turn.
...
- Aid is now implemented as a playable prepare flow: `AidAction` provides ally-in-reach targeting validation + prepare execution, `PlayerActionExecutor` wires `CanPrepareAid`/`TryExecuteAid`, `TargetingController` supports explicit `TargetingMode.Aid` (ally targeting), and Action Bar / hotkey input can enter Aid targeting mode. Aid check resolution/bonus application is still pending in strike/skill execution paths.

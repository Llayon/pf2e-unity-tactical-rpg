...
- PlayMode regression now covers multi-round movement/AI/condition-tick flow, blocked-turn recovery, sticky-target lock behavior, ranged concealment/cover logic, Shield Block reaction prompts, and Delay planned/manual return-window core flows; full matrix coverage for spells/visibility states and pointer-click UI path tests is still pending.
...
1. Add pointer-level PlayMode coverage for Delay UI controls: insertion marker click path, Action Bar `Delay` toggle, and `ReturnNow`/`Skip` button interaction wiring.

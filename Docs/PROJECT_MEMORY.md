...
- `SampleScene` now also has a cleaner opening trap beat: the fighter's code-authored spawn cell is aligned with the `Hook Snare` approach lane, so the primary tracked encounter can demonstrate a readable trap trigger in the first rounds without relying on AI-only shove/reposition behavior.
